...
// ── Football team colourways ──────────────────────────────────────────────
// Bright, saturated shells so the two sides read apart under the Tron-style
// field lighting. Linear-space values tuned for ACES tone mapping.
const TEAM_RED_SHELL = { color: [0.85, 0.04, 0.10], roughness: 0.35, metalness: 0.0 };     // ~#ff2244
const TEAM_RED_DARK = { color: [0.45, 0.02, 0.06], roughness: 0.5, metalness: 0.0 };       // deep crimson
const TEAM_RED_ACCENT = { color: [1.0, 0.25, 0.28], roughness: 0.4, metalness: 0.0 };      // bright red accent
const TEAM_BLUE_SHELL = { color: [0.04, 0.16, 0.85], roughness: 0.35, metalness: 0.0 };    // ~#2266ff
const TEAM_BLUE_DARK = { color: [0.02, 0.08, 0.45], roughness: 0.5, metalness: 0.0 };      // deep navy
const TEAM_BLUE_ACCENT = { color: [0.25, 0.48, 1.0], roughness: 0.4, metalness: 0.0 };     // bright blue accent
...
export const VARIANTS = {
  ...
  // 3v3 football: red team. Vivid crimson shells with dark joints, cream
  // accents on the soles so the feet stay visible on the green pitch.
  team_red: {
    headDome: TEAM_RED_SHELL,
    facePlate: TEAM_RED_DARK,
    trim: TEAM_RED_ACCENT,
    beakUpper: TEAM_RED_ACCENT,
    beakLower: TEAM_RED_ACCENT,
    tongue: TEAM_RED_DARK,
    eyeRing: CLEAN_YELLOW,
    lens: LENS,
    bodyShell: TEAM_RED_SHELL,
    sideShells: TEAM_RED_SHELL,
    legShells: TEAM_RED_SHELL,
    feet: TEAM_RED_DARK,
    soles: CREAM,
    hips: GRAY,
    mechDark: DARK,
  ...
  // 3v3 football: blue team. Deep blue shells with navy joints, cream soles.
  team_blue: {
    headDome: TEAM_BLUE_SHELL,
    facePlate: TEAM_BLUE_DARK,
    trim: TEAM_BLUE_ACCENT,
    beakUpper: TEAM_BLUE_ACCENT,
    beakLower: TEAM_BLUE_ACCENT,
    tongue: TEAM_BLUE_DARK,
    eyeRing: CLEAN_YELLOW,
    lens: LENS,
    bodyShell: TEAM_BLUE_SHELL,
    sideShells: TEAM_BLUE_SHELL,
    legShells: TEAM_BLUE_SHELL,
    feet: TEAM_BLUE_DARK,
    soles: CREAM,
    hips: GRAY,
    mechDark: DARK,
...
// The four retail robots only (VARIANT_NAMES also carries the two football
// team colourways, which the sandbox randomiser must never hand out).
export const SANDBOX_VARIANT_NAMES = VARIANT_NAMES.filter(
  (n) => n !== "team_red" && n !== "team_blue",
);
...
// Football team colourways are deliberately kept OUT of VARIANT_SWATCH_HEX:
// that table drives the single-duck sandbox picker (Hud.jsx Quickbar) and is
// shared verbatim with the pollen-website landing, so it must stay the four
// retail robots. The 3v3 match reads its two sides from this dedicated table
// instead (same sRGB targets as the material specs above).
export const TEAM_VARIANT_NAMES = ["team_red", "team_blue"];
export const TEAM_SWATCH_HEX = {
  team_red: "#ff2244",
  team_blue: "#2266ff",
};

export const randomVariantName = () =>
  SANDBOX_VARIANT_NAMES[Math.floor(Math.random() * SANDBOX_VARIANT_NAMES.length)];
